...
function apply(ctx) {
  ...
  // ── 状态信号（参照 whale-on-desk：订阅持久会话事件，而非 DOM 文本猜测）──
  // DSH 的 session/event 事件携带回合/流式块/工具调用等结构化状态；浏览器端
  // 轮询 /dsh-status-glow/state 获取，作为按状态分流与按池特效的权威依据。
  let hostStatus = 'default' // thinking | tool | command | default
  let hostTool = null        // 最近一次工具名（tool/call 的 name）
  let hostTs = 0
  const COMMAND_TOOLS = /^(?:bash|pwsh|powershell|shell|cmd|terminal|sh|zsh|fish|nu|ps1?)$/

  const foldStatus = (event) => {
    const type = event && event.type
    const d = (event && event.data) || {}
    try {
      if (type === 'assistant/chunk') {
        // 非 text-delta 的块 = 推理/思考
        if (d.chunk && d.chunk.type !== 'text-delta') { hostStatus = 'thinking'; hostTool = null; hostTs = Date.now() }
      } else if (type === 'tool/call') {
        const name = String(d.name || 'tool').toLowerCase()
        hostStatus = COMMAND_TOOLS.test(name) ? 'command' : 'tool'
        hostTool = name
        hostTs = Date.now()
      } else if (type === 'tool/result') {
        // 工具结束后通常继续推理
        if (hostStatus === 'tool' || hostStatus === 'command') { hostStatus = 'thinking'; hostTool = null; hostTs = Date.now() }
      } else if (type === 'turn/end') {
        hostStatus = 'default'; hostTool = null; hostTs = Date.now()
      }
    } catch (err) { /* 事件形状异常不影响插件 */ }
  }
  ...
  // Live status endpoint: browser half polls this (every ~1s) to learn the
  // current phase from durable session events (thinking/tool/command).
  disposers.push(ctx.webServer.register({
    kind: 'exact',
    path: '/dsh-status-glow/state',
    handler: (_req, res) => {
      res.writeHead(200, {
        'Content-Type': 'application/json; charset=utf-8',
        'Cache-Control': 'no-store',
      })
      res.end(JSON.stringify({ status: hostStatus, tool: hostTool, ts: hostTs }))
    },
  }))
