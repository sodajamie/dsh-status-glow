...
(function (global) {
  ...
  function normalizeConfig(input) {
    ...
    cfg.poolEffects = normalizePoolEffects(src.poolEffects)
  ...
  function nearbyText(cand, excludeText) {
    try {
      ...
      if (!el) return ''
      var out = ''
      // 祖先链 + 兄弟元素文本（加深到 6 层，扩大捕获阶段标签/工具摘要）
  ...
  function loadPools() {
    ...
    registerStatusType('thinking', { detect: function (t) { return /思考|think|推理|analyzing|reasoning/i.test(t) }, poolId: 'pool:thinking' })
    registerStatusType('tool', { detect: function (t) { return /工具|tool|调用|executing|run (the )?tool|正在使用/i.test(t) }, poolId: 'pool:tool' })
    registerStatusType('command', { detect: function (t) { return /命令|command|shell|终端|terminal|执行|exec/i.test(t) }, poolId: 'pool:command' })
  }
  ...
  function applyFresh(rec, cand) {
    if (currentCfg.usePools) {
      currentStatus = detectStatus(cand, rec.orig)
  ...
  function refreshStatus(rec, el, forceDraw) {
    ...
    var status = detectStatus({ el: el, node: rec.node }, rec.applied)
    ...
    try {
      ...
      var payload = {
        ...
        chosen: {
          ...
          rect: chosen.rect,
          computed: cs,
          status: detectStatus(chosen, origTexts.get(chosen.el) ? origTexts.get(chosen.el).applied : null),
