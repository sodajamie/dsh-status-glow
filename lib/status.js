...
(function (global) {
  ...
  // 按权重展开为 bag 并洗牌；相邻重复副本保留一个（保证队列内无相邻同文案）。
  function weightedShuffle(candidates) {
    var bag = []
  ...
  function draw(poolId) {
    if (!queues[poolId] || queues[poolId].length === 0) {
      ...
      if (!pool) return null
      var q = weightedShuffle(pool.candidates)
      if (q.length > 1 && q[0] === lastDrawn[poolId]) {
        var tmp = q[0]
        q[0] = q[q.length - 1]
        q[q.length - 1] = tmp // 跨轮不重复
      }
      queues[poolId] = q
    }
