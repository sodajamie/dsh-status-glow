...
(function (global) {
  ...
  // rollEl 跟踪当前滚动元素：rollTimer 被新一轮 rollTo 打断时（换目标/配置变更），
  // 旧元素的定时器回调不再执行，必须在此把它移出 rolling，否则它永远留在集合里，
  // sweepGlow 永远跳过它（内存泄漏 + 辉光无法清理）。
  var rollEl = null
  function rollTo(rec, el, poolId, finalText, style) {
    if (rollTimer) {
      clearInterval(rollTimer); rollTimer = null
      if (rollEl && rollEl !== el) rolling.delete(rollEl)
      rollEl = null
    }
    rolling.add(el)
    rollEl = el
    ...
    rollTimer = setInterval(function () {
      if (!el.isConnected || !origTexts.has(el)) {
        clearInterval(rollTimer); rollTimer = null; rollEl = null
        rolling.delete(el)
        return
      }
      if (steps >= maxSteps) {
  ...
  // 元素断开或文本被应用改写后，除 unglow 外必须同步删除 origTexts/rolling 记录，
  // 否则长会话中 Map/Set 持续累积 stale DOM 引用（内存泄漏，元素无法被 GC）。
  function sweepGlow() {
    glowed.forEach(function (el) {
      if (rolling.has(el)) return
      if (!el.isConnected) { unglow(el); origTexts.delete(el); return }
      var rec = origTexts.get(el)
      var applied = rec ? rec.applied : currentCfg.text
      if (String(el.textContent).indexOf(applied) === -1) { unglow(el); origTexts.delete(el) }
    })
  }
