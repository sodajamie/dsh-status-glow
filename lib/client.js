...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // textShadow 结构化比较键：false/string/array(string|{color,x,y,blur}) 归一化为
    // 可比较字符串。数组按引用 === 比较永远不相等，导致霓虹类特效的池下拉永远误显示
    // 「跟随全局」（配置实际生效，仅回显错误）。
    function shadowKey(v) {
      if (v === undefined || v === null) return null;
      if (v === false || v === "none" || v === "") return "off";
      ...
      if (Array.isArray(v)) {
        return "a:" + v.map((l) => {
          ...
          if (l && typeof l === "object") {
            ...
            const blur = typeof l.blur === "number" ? l.blur : 8;
            return x + "," + y + "," + blur + "," + String(l.color || "").toLowerCase();
          }
          return "";
        }).join("|");
      }
    ...
    function effectConfigEquals(a, b) {
      ...
      return norm(a.color) === norm(b.color) &&
             shadowKey(a.textShadow) === shadowKey(b.textShadow) &&
