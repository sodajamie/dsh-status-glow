...
| 版本 | 日期 | 说明 |
|---|---|---|
| `0.2.0` | 2026-09-01 随机文案池按状态分流（思考/工具/命令/兜底 + 加权无相邻重复）+ 同状态 5s / 状态交替立即换文 + 老虎机滚动动画 + 按池专属特效 + 池整表编辑 + 自定义池内容/特效预设 + 状态强制预览 + 宿主会话事件检测（session/event 折叠 + `/state` 轮询）+ 宿主文件持久化（跨重启不丢）+ 状态按「最后出现位置」分类；出厂默认 = 当前调校效果（白字无辉光 + 四池特效 + 池文案开箱即用） |
