...
把 DSH 界面里表示「正在工作」的**最下方总状态** **`Deep diving...`** 改为自定义文案（出厂默认「大肥鲸鱼正在深度烧烤...( ˊ꒳ˋ )ₚ✧」），文字样式可自定义：**纯色 / 从左到右渐变**（`background-clip:text`）、辉光可关/可多层合成、动画（内置流动渐变）；随机文案池按状态分流，每状态可配专属特效。
...
> v0.2.0（整合发布）：随机文案池按状态分流 + 按池特效 + 宿主会话事件检测 + 宿主文件持久化，出厂默认 = 当前调校效果。
...
| 字段 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `text` | string | `'大肥鲸鱼正在深度烧烤...( ˊ꒳ˋ )ₚ✧'` | 替换后的完整静态文本（`usePools` 关闭/未命中池时兜底）；空串/非字符串回退默认 |
| `gradientColors` | string[] | `null` | 从左到右渐变（合法 hex，≥2 色才生效，非法项被过滤）；与 `color` 互斥，**渐变优先**；出厂默认走纯色路径，渐变由各池特效提供 |
| `color` | string | `'#ffffff'` | 纯色（hex / `rgb()` / `hsl()` / 命名色）；仅当无有效渐变时生效 |
| `textShadow` | `false` \| string \| array | `false` | `false`/`'none'`/`''`=关闭；`'soft-white'`=柔和白光；字符串原串直接透传；数组=多层（元素可为原串或 `{color,x,y,blur}`，`x/y/blur` 默认 `0/0/8px`） |
| `animation` | `null` \| string | `null` | `null`/`false`=关闭；`'flow'`=内置流动渐变（`dshsg-flow`）；其他字符串作为 CSS `animation` 原串透传 |
| `usePools` | boolean | `true` | 随机文案池开关：`true` 时按状态（思考/工具/命令…）从 `lib/text-pools.js` + 自定义内容抽取；`false` 使用静态 `text` |
| `poolRepeat` | boolean | `false` | 随机是否允许相邻重复：`true`=真随机；`false`=洗牌式无相邻重复（默认，含跨轮队首交换） |
| `poolCustoms` | object | `{}` | 各池自定义文案：`{ thinking: [{ text, weight }], … }`，追加到对应池并参与加权抽取（默认权重 1） |
| `poolOverrides` | object | `{}` | 各池整表覆盖：`{ thinking: [{ text, weight }], … }`，存在时**完全替换**该池文案（优先级最高；设置页增删改落盘于此） |
| `poolEffects` | object | 四池出厂特效 | 按池特效：`{ thinking: { 样式片段 }, … }`，开启 `usePools` 且命中该状态时用池特效替代全局特效；未显式配置的池回退出厂特效（思考=彩虹渐变+流动动画、工具=金黄渐变+柔和白光、命令=白字+蓝紫多层辉光、兜底=白字无辉光） |
| `effects` | array | `[]` | 自定义特效预设：`[{ key, label, config }]`，`config` 为样式片段（`gradientColors`/`color`/`textShadow`/`animation`），出现在设置页特效下拉框 |

所有字段提供默认值、类型校验与非法回退（例如 `gradientColors:['#ff0000']` 只有 1 色 → 回退默认纯色 `#ffffff`；`textShadow:42` → 回退柔和白光）。`window.__dshStatusGlow.getConfig()` 可读取当前归一化后的配置。

### 调用示例

```js
// 1) 文本替换：只改文案，其余保持默认（白字无辉光 + 按池特效）
window.__dshStatusGlow.configure({ text: '正在深度烧烤...' })

// 2) 红蓝渐变：文字本体红→蓝渐变（覆盖默认纯色路径）
...
- **随机文案 + 允许重复**（开关）：按状态分流抽取；「允许重复」默认关（洗牌相邻不重复），开启则真随机；
- **各池卡片**（文案池区）：每个状态一张卡片——列表展示该池全部文本（内置 + 自定义），行内编辑（改文本/权重）、删除、添加，特效下拉为该状态指定专属特效，「恢复默认」一键还原；编辑即时生效；
- **应用**按钮：即时调用 `window.__dshStatusGlow.configure(...)`，**无需重启 DSH**，状态元素立即刷新样式与文本。

配置持久化：设置页每次变更即时写入**宿主文件** `~/.dsh/dsh-status-glow-config.json`（端口无关，跨 DSH 重启保留），并同步 `localStorage` 缓存——重开设置、刷新页面、重启 DSH 后自定义均自动恢复。
...
- **随机抽取**：Fisher-Yates 加权洗牌队列；默认**相邻不重复**（`poolRepeat: false`，含跨轮队首交换）；设 `true` 时允许相邻重复（真随机）；
- **权重**：`weight: 2` 的文案出现频率约为 `weight: 1` 的两倍（默认 1）；
- **按池特效**（设置页每池卡片「特效」下拉，或配置 `poolEffects`）：每个状态可指定专属特效，覆盖全局特效：
...
- **池整表编辑**（设置页每池卡片，或配置 `poolOverrides`）：完全替换该池文案（内置 + 自定义同表维护）：

```js
window.__dshStatusGlow.configure({
  poolOverrides: { tool: [{ text: '只留这一条工具文案', weight: 1 }] },
})
```

- **自定义特效**（设置页「自定义特效」或配置 `effects`，出现在特效下拉框与各池特效下拉）：
...
- 单元测试：`node test\status.test.mjs`（覆盖匹配/评分/选择/配置归一化与非法回退/随机文案池/自定义池与特效/按池特效，51 项断言）。
...
| 版本 | 日期 | 说明 |
|---|---|---|
| `0.2.0` | 2026-09-01 | 整合发布版（汇集此前 0.2~0.9 全部更新）：随机文案池按状态分流（思考/工具/命令/兜底 + 加权无相邻重复）+ 同状态 5s / 状态交替立即换文 + 老虎机滚动动画 + 按池专属特效 + 池整表编辑 + 自定义池内容/特效预设 + 状态强制预览 + 宿主会话事件检测（session/event 折叠 + `/state` 轮询）+ 宿主文件持久化（跨重启不丢）+ 状态按「最后出现位置」分类；出厂默认 = 当前调校效果（白字无辉光 + 四池特效 + 池文案开箱即用） |
| `0.1.0` | 2026-08-28 | 初始版：文本替换 + 渐变彩虹 + 柔和白光辉光 + 设置页快捷入口 + 精确命中最下方总状态（开发期版本，未单独打 tag） |

完整变更记录见 [CHANGELOG.md](./CHANGELOG.md)；git 标签：`v0.2.0`。
